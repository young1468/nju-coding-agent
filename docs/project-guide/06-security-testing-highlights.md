...
- 回归检查：完整 `pytest`（当前 56 项）和 Tkinter 无交互启动。
...
| 确定性反馈 | `verification` 分类器识别测试失败、超时和命令错误 | 让模型获得可直接消费的客观信号 |
| 进度可视化 | 状态条显示阶段、Step、压缩次数和最终状态 | 长任务可观察、历史可恢复 |
| 分层长期记忆 | 全局/项目 Markdown + JSON 索引，按任务检索 | 跨会话复用稳定偏好和项目知识 |

## 五、边界与未来

已实现的是一条完整、可测试的本地 coding workflow，借鉴 Pi 的是上下文工程、输出控制、会话持久化和稳定/动态上下文分层。Skills 懒加载、Session Tree 分支、流式响应、命令沙箱、向量记忆和多 Agent 编排仍属于未来扩展，不应误称为当前能力。
