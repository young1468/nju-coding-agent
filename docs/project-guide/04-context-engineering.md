...
长期记忆是跨会话保存的稳定事实，位于用户目录或 workspace 的 `.coding-agent/memory.md`；compaction 是单个 JSONL 会话内对旧消息的阶段性摘要。每轮请求先按任务检索有限的长期记忆，再叠加当前会话的摘要和近期消息。这样用户偏好、项目约定不会随着某个会话结束而丢失，同时临时测试输出也不会被自动写入长期记忆。
